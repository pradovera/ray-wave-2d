...
from angular_weight import (ray, ray_r, ray_d, ray_list,
                            piecewise_linear_weights,
                            build_angular_weight_reflect,
                            build_angular_weight_diffract)

def build_ray_sequence(D, u0_c, u0_supp, t_max, model_diffraction = True,
                       weights_cutoff = None, verbose = 0):
    ...
    # if model_diffraction == True, model diffraction effects
    timetable = np.empty((0, D.nedges + D.npoints * model_diffraction))
    angle_supp_r = [] # list of reflection support angles
    angle_supp_d = [] # list of diffraction support angles
    rays = [] # list of rays
    ...
    while c_t - u0_supp <= t_max:
        ...
        if idx_i is None: # first ray
            ...
            ignore_p = [idx_j]
            diff_info = angle_supp_d[idx_i][idx_j]
        ...
        if not ignore_ray_loc:
            if wave_kind == "BASE":
                ...
                vs, ps = build_angular_weight_reflect(D, ray_loc,
                                                      weights[ray_loc.gen],
                                                      r_coeff)
            elif wave_kind == "DIFFRACTION":
                # -1 = Dirichlet, 1 = Neumann
                r_coeff_i = 2 * D.BCs(ray_loc.ignore[0]) - 1
                r_coeff_o = 2 * D.BCs(ray_loc.ignore[len(ray_loc.ignore)
                                                   - 1]) - 1
                vs, ps = build_angular_weight_diffract(ray_loc,
                                                       rays[ray_loc.gen],
                                                       weights[ray_loc.gen],
                                                       r_coeff_i, r_coeff_o)
            if ps:
            ...
            for j in range(len(ps) - 2, -2, -2):
                if np.abs(ps[j + 1] - ps[j]) < EPS:
                    ...
                    vs.pop(j), ps.pop(j)
            weights_loc = piecewise_linear_weights(vs, ps)
            ignore_ray_loc = np.max(np.abs(vs)) < weights_cutoff(ray_loc.s_t)
        if not ignore_ray_loc:
            ...
            if model_diffraction:
                # cast new ray to points
                angle_supp_d_loc = []
                for j in range(D.npoints):
                    angle_supp_d_locj = D.is_diffraction_point(c_x, j)
                    if angle_supp_d_locj[0] == -3: # diff from integer angle
                        angle_supp_d_locj = [0]
                    if (len(angle_supp_d_locj) == 3
                    and wave_kind == "REFLECTION" and j in D.edges[idx_j]):
                        # if j is part of the edge supporting reflection,
                        #   with a convex corner, its diffraction is
                        #   already accounted for
                        angle_supp_d_locj = [0]
                    if (len(angle_supp_d_locj) == 2
                    and wave_kind == "REFLECTION" and j in D.edges[idx_j]):
                        idx_edge_i = np.where(D.adjacency[j, :] == -1)[0][0]
                        if idx_edge_i in D.edges[idx_j]:
                            angle_supp_d_locj[0] = 3 # light first
                        else:
                            angle_supp_d_locj[0] = 4 # shadow first
                    angle_supp_d_loc += [angle_supp_d_locj]
                angle_supp_d += [angle_supp_d_loc]
            ...
            if model_diffraction:
                ...
                for j, t in enumerate(t_arrival_p):
                    if (j not in ignore_p and t is not None
                                          and angle_supp_d[-1][j][0]):
